...
import type { LeaderboardMetric, LeaderboardPeriod } from '@poker/shared';
import { renderWithClient } from '../../test/harness';
import { makeEntry, makeProfile } from '../fixtures';
import { FETCH_LIMIT, LeaderboardScreen, SHOWN } from './LeaderboardScreen';
...
describe('LeaderboardScreen', () => {
  it('fetches deep enough to find you and highlights your row', async () => {
    const leaderboard = vi.fn(async () => board(8, { me: 5 }));
    renderWithClient(<LeaderboardScreen />, { api: { leaderboard } });
    expect(leaderboard).toHaveBeenCalledWith('net_profit', 'all', FETCH_LIMIT);
  ...
  it('says when you are below the whole fetched board', async () => {
    renderWithClient(<LeaderboardScreen />, { api: { leaderboard: async () => board(FETCH_LIMIT) } });
    expect(await screen.findByText(`You're outside the top ${FETCH_LIMIT}. Keep playing.`)).toBeInTheDocument();
  });

  it('pins your row below the list when you are outside the shown rows', async () => {
    const leaderboard = vi.fn(async () => board(40, { me: 33 }));
    renderWithClient(<LeaderboardScreen />, { api: { leaderboard } });
    const pinned = await screen.findByRole('group', { name: 'Your position' });
    expect(within(pinned).getByRole('button', { name: /Alice/ })).toHaveTextContent('33');
  ...
  it('says when you are not on the board at all', async () => {
    renderWithClient(<LeaderboardScreen />, { api: { leaderboard: async () => board(4) } });
  ...
  it('only offers "This week" for weekly metrics', async () => {
    const leaderboard = vi.fn(async (_m: LeaderboardMetric, _p?: LeaderboardPeriod) => board(3));
    renderWithClient(<LeaderboardScreen />, { api: { leaderboard } });
    await screen.findByRole('list', { name: 'Top three' });

    await userEvent.click(screen.getByRole('radio', { name: 'This week' }));
    expect(leaderboard).toHaveBeenLastCalledWith('net_profit', 'week', FETCH_LIMIT);
    ...
    expect(leaderboard).toHaveBeenLastCalledWith('bankroll', 'all', FETCH_LIMIT);

    // Back to a weekly metric: the weekly choice is remembered.
    await userEvent.click(screen.getByRole('radio', { name: 'Hands won' }));
    expect(leaderboard).toHaveBeenLastCalledWith('hands_won', 'week', FETCH_LIMIT);
  });
  ...
  it('shows tied ranks and podium places', async () => {
    ...
    renderWithClient(<LeaderboardScreen />, { api: { leaderboard: async () => entries } });
  ...
  it('opens the profile card from a row', async () => {
    ...
    renderWithClient(<LeaderboardScreen />, { api: { leaderboard: async () => board(3), profile } });
    await userEvent.click(await screen.findByRole('button', { name: /1st: Player 1/ }));
    expect(profile).toHaveBeenCalledWith('x0');
    expect(await screen.findByRole('dialog')).toBeInTheDocument();
  });

  it('has an empty state', async () => {
    renderWithClient(<LeaderboardScreen />, { api: { leaderboard: async () => [] } });
